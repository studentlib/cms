...
class account extends admin
{
    ...
    public function __construct()
    {
        ...
        if(empty($platform['platform'])){
            ...
            $this->_condition=' AND `channel` IN ('.$str.')';
        }
    }
    ...
    public function ltv()
    {
        ...
        $result = $this->_daily_ltv->select($time.$this->_condition);
    ...
    public function base()
    {
        ...
        $ed = date('Y-m-d');//$_POST['et'];
        $time = sprintf("`time` between '%s' AND '%s'", $st, $ed);
        $result = $this->_daily_base->select($time.$this->_condition);
    ...
    {
        $ret = array('code' => 0, 'msg' => '查询为空', 'content' => '');
        $condition = '*';//`time`,`num`,`one_amount`,`one_ltv`';//查询内容
        $st = $_POST['st'];
        $ed = $_POST['ed'];
        $ch = empty($_POST['ch']) ? '' : 'AND `channel` =' . "'" . $_POST['ch'] . "'";
        $sql = sprintf("SELECT %s FROM GMdaily_ltv where `time` BETWEEN '%s' AND '%s' %s".$this->_condition, $condition, $st, $ed, $ch);
        $this->_daily_ltv->query($sql);
        $result = $this->_daily_ltv->fetch_array();//var_dump($result);
    ...
    public function findbase()
    {
        ...
        $sid = empty($_POST['sid']) ? '' : 'AND `game`=' . "'" . $_POST['sid'] . "'";
        $sql = sprintf("SELECT %s FROM GMdaily_base where `time` BETWEEN '%s' AND '%s' %s %s".$this->_condition, $condition, $st, $ed, $ch, $sid);
        //var_dump($sql);
    ...
    public function exportLtv()
    {
        ...
        $sql = sprintf('mysql -uroot GM_txmix -e "select * from GMdaily_%s where time between %s and %s  %s %s;" > %s', $file, $st, $ed, $ch, $sid, $address);
        //$sql=sprintf('mysql -uroot GM_txmix -e "select * from GMdaily_ltv;" > %s',$address);
        //var_dump($sql);
        exec($sql);
