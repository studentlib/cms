...
$(document).ready(function(){
	...
	$('#save').click(function(){
	    ...
	    $.post('index.php?m=active&c=active&a=update_login',post,function(data){
	        alert(data.msg);
	    },'json');
    }); 
	...
	$('#save_all').click(function(){
       ...
       $.post('index.php?m=active&c=active&a=sync_login',post,function(data){
            alert(data.msg);
        },'json');
	});
...
<input type="hidden" id="Days_<?php echo $attr['Days'];?>" value="<?php echo $attr['Days'];?>"/>
<label>标题</label>
<input id="Des_<?php echo $attr['Days'];?>" type="text" disabled="disabled" value="<?php echo $attr['Des'];?>"/>
<?php 
$i=0;
foreach ($items as $k=>$va) {
$i++;
?>
<label>返还物品<?php echo $k+1;?></label>
<!-- <input type="text" name="" id="Items_<?php echo $k+1;?>_<?php echo $attr['Days'];?>" class="item" style="width: 80px"; 
value='<?php echo $config[$va[0]]['name'];?>' onblur="isItemEexit('Items_<?php echo $k+1;?>_<?php echo $attr['Days'];?>')"> -->
<input list="company" type="text" name="" id="ItemReward_<?php echo $k+1;?>_<?php echo $attr['Days'];?>" class="ItemReward" style="width: 80px";
value='<?php echo $config[array_keys($va)[0]]['name'];?>' onblur="isItemEexit('ItemReward_<?php echo $k+1;?>_<?php echo $attr['Days'];?>')"> 
<datalist id="company">
<?php  
...
<input type="text" name="" id="Count_<?php echo $k+1;?>_<?php echo $attr['Days'];?>" class="count" style="width: 50px";
