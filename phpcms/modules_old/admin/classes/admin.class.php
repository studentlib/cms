...
//        	}
	...
	/*后台操作权限*/
//	final public function check_priv_admin() {
//		$ptiv= pc_base::load_config('mvc','c');
//		if($_SESSION['roleid'] ==16 ||$_SESSION['roleid'] ==17||$_SESSION['roleid'] ==8){
//			if(in_array(ROUTE_A, $ptiv)) return array('code'=>0,'msg'=>'您没有权限操作该项');
//			if(strpos(ROUTE_A,'sync')!==false) return array('code'=>0,'msg'=>'您没有权限操作该项');
//			if(strpos(ROUTE_A,'update')!==false) return array('code'=>0,'msg'=>'您没有权限操作该项');
//			}
//	}
	/**
	 * 
		...
		extract($setconfig);
 		if($admin_log==1){
 			...
 			if($action == '' || strchr($action,'public') || $action == 'init' || $action=='public_current_pos') {
				...
				$log->insert(array('module'=>ROUTE_M,'file'=>$file,'username'=>$username,'data'=>json_encode($_REQUEST),'userid'=>$userid,'action'=>$action, 'querystring'=>$url,'time'=>$time,'ip'=>$ip));
			}
	  	}
	}
    ...
    protected  function getRedis($ip,$index,$port=6379)
    {
        ...
        try {
            ...
            //exit('本区redis数据库已关闭，请换区测试.');//print_r($e)
	    return '1';
        }
    ...
    protected  function get_config_file()
    {
        ...
        foreach ($items as $k=>$v) 
        {
            ...
            {
                $config[(string)$v['ItemID']]=
                array(
                	'id'=>(string)$v['ItemID'],
                	'name'=>$lang_config[(string)$v['ItemName']],
                	'type'=>(string)$v['ItemType']
                	);
            }
        }
        return $config;
    ...
    protected  function get_config()
    {
        ...
        foreach ($items as $k=>$v)
        {
            if(isset($v['ItemID'])&&isset($v['ItemID']))
            {
            $config[(string)$v['ItemID']]=array(
                    'id'=>(string)$v['ItemID'],
                    'name'=>$lang_config[(string)$v['ItemName']],
                    'type'=>(string)$v['ItemType']);
            }
    ...
    protected  function get_config_item()
    {
        ...
        foreach ($items as $k=>$v)
        {
            if(isset($v['ItemID'])&&isset($v['ItemName']))
            {
            $config[$lang_config[(string)$v['ItemName']]]=array(
                    'id'=>(string)$v['ItemID'],
                    'name'=>$lang_config[(string)$v['ItemName']],
                    'type'=>(string)$v['ItemType']);
            }
...
//             return $config;
    ...
    protected  function limitshop()
    {
        $limititems=simplexml_load_file(PHPCMS_PATH.'/statics/config/ActivityShopItem.xml');
        $limititem=array();
        foreach ($limititems->row as $k=>$v) {
            $limititem[(string)$v['ID']]=array('ID'=>(string)$v['ID'],'ItemId'=>(string)$v['ItemId'],'PriceFormal'=>(string)$v['PriceFormal'],'PriceNow'=>(string)$v['PriceNow']);
        }
        return $limititem;
    }
    ...
    protected  function get_server_config()
    {
        ...
        $admin_group=array(1,8,16,17,18);
        $ret=array();
