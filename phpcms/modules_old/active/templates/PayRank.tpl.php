...
$(document).ready(function(){
	...
	$('#save').click(function(){
		...
		$("#edit_lu :input").each(function(k,v){
		 ...
		 post.sid=$('#servers').val();
		    ...
		    $.post('index.php?m=active&c=active&a=update_PayRank',post,function(data){
		        alert(data.msg);
				location.reload();
		    },'json');
    }); 
	...
	$('#save_all').click(function(){
       ...
       $.post('index.php?m=active&c=active&a=sync_PayRank',post,function(data){
            alert(data.msg);
        },'json');
	});
});
...
<fieldset>
<legend>Sửa tổng kết hạng nạp</legend>
<br/>
<?php 
...
//     if($k==1){
//         $days=$RankOverDay<=3? 3 : $RankOverDay;
//         echo '<span >';
//         echo "<input id='ID_".$v['ID']."' type='hidden' value='".$v['ID']."' />";
//         echo "<input id='Key_".$v['ID']."' type='hidden' value='".$v['Key']."'/>";
//         echo "<span>&nbsp".$v['Des'].$str."&nbsp:&nbsp</span><input id='Value_".$v['ID']."' value='".$days."' style='width: 60px;'/>";
...
<label>Xếp hạng&nbsp:&nbsp</label>
