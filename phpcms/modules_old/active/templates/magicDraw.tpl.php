...
$(document).ready(function(){
	...
	$('#save').click(function(){
	    ...
	    $.post('index.php?m=active&c=active&a=update_magicDraw',post,function(data){
	        alert(data.msg);
	    },'json');
    }); 
	...
	$('#save_all').click(function(){
       ...
       $.post('index.php?m=active&c=active&a=sync_magicDraw',post,function(data){
            alert(data.msg);
        },'json');
	});
...
<label>Doanh thu Id</label>
<input type="text" id="Id_<?php echo $attr['Id'];?>" value="<?php echo $attr['Id']?>" disabled="disabled" style="width:50px;"/>
<label>Số Ngọc hiếm</label>
<input type="text" id="ArtifactTimes_<?php echo $attr['Id'];?>" value="<?php echo $attr['ArtifactTimes']?>" style="width:50px;"/>
<?php 
$i=0;
foreach ($items as $k=>$va) {
$i++;
?>
<label>Thưởng đạo cụ<?php echo $i;?></label>
<!-- <input type="text" name="" id="Items_<?php echo $k+1;?>_<?php echo $attr['Id'];?>" class="item" style="width: 80px"; 
value='<?php echo $config[$va[0]]['name'];?>' onblur="isItemEexit('Items_<?php echo $k+1;?>_<?php echo $attr['Id'];?>')"> -->
<input list="company" type="text" name="" id="Items_<?php echo $k+1;?>_<?php echo $attr['Id'];?>" class="item" style="width: 80px;" 
value='<?php echo $config[array_keys($va)['0']]['name'];?>' onblur="isItemEexit('Items_<?php echo $k+1;?>_<?php echo $attr['Id'];?>')">
<datalist id="company">
<?php  
