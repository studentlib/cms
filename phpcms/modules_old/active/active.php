...
class active extends admin {
	...
	public function index() {
		...
		$sid = isset ( $_SESSION ['sid'] ) && $_SESSION ['sid'] != 0 ? $_SESSION ['sid'] : 1;
	...
	public function exportConfig() {
		...
		if (! empty ( $sid )) {
			...
			$default = $this->_active->get_one ( array (
					'server_id' => $sid 
			) );
			if ($default) {
				$dir = "xml/" . $sid;
		...
		if (isset ( $_POST ['sid'] ) && isset ( $_POST ['servers'] )) {
			if (is_array ( $_POST ['servers'] )) {
				...
				if ($content) {
					...
					$ret ['msg'] = 'Đồng bộ thành công';
				} else {
					$ret ['code'] = 2;
					$ret ['msg'] = 'xin vui lòng lưu';
				}
			} else {
				$ret ['code'] = 3;
				$ret ['msg'] = 'Chọn server cần đồng bộ';
			}
		} else {
			$ret ['code'] = 1;
	...
	public function update() {
		...
		if (isset ( $_POST ['content'] ) && isset ( $_POST ['sid'] )) {
			...
			$default = $this->_active->get_one ( '`server_id`=' . $sid );
			if (! $default) {
				$this->_active->insert ( array (
						'server_id' => $_POST ['sid'],
						'content' => $xml 
				) );
			} else {
				$this->_active->update ( array (
						'server_id' => $_POST ['sid'],
						'content' => $xml 
				), '`server_id`=' . $sid );
			}
	...
	public function findXml_array($uid, $items, $templet) {
		$array = array ();
		$sid = isset ( $_SESSION ['sid'] ) ? $_SESSION ['sid'] + $uid : $uid + 1;
	...
	private function __get_limit_item() {
		...
		foreach ( $limititems->row as $k => $v ) {
			$limititem [( string ) $v ['ID']] = array (
					'ID' => ( string ) $v ['ID'],
					'Des' => $lang_config [( string ) $v ['Name']],
					'ItemId' => ( string ) $v ['ItemId'],
					'PriceFormal' => ( string ) $v ['PriceFormal'],
					'PriceNow' => ( string ) $v ['PriceNow'] 
			);
		}
		return $limititem;
	}
