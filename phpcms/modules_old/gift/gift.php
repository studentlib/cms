...
class gift extends admin {
	...
	if(empty($data['uid'])){
	  $str1=$this->_used->listinfo("`code`='".$data['code']."'",'',1,100);
	  $str2=$this->_gift->select("`key`='".$data['code']."'",'`id`,`key`,`uid`,`no`,`channel`');
	  $str=array_merge($str1,$str2);
	  $ret['str']=$str;
	}elseif(empty($data['code'])){
	  $str1=$this->_used->listinfo("`uid`='".$data['uid']."'",'',1,100);
	  $str2=$this->_gift->select("`uid`='".$data['uid']."'",'`id`,`key`,`uid`,`no`,`channel`');
	  $str=array_merge($str1,$str2);
	  $ret['str']=$str;
	}else{
	  $str1=$this->_used->select("`uid`='".$data['uid']."' and `code`='".$data['code']."'");
	  $str2=$this->_gift->select("`uid`='".$data['uid']."' and `key`='".$data['code']."'",'`id`,`key`,`uid`,`no`,`channel`');
	  $str=array_merge($str1,$str2);
	  $ret['str']=$str;
	  if(empty($str)){
	        $ret['code']=110;
		$ret['msg']='Người dùng đã không sử dụng mã đổi quà';
